...
def process_age_dual(age, delJz_obs):
    # Non-biased
...
with h5py.File(h5_file, "a") as h5f:
    with open(outfile, "a", newline="") as f:
        ...
        for cluster_name in tqdm(clusters[np.isin(clusters,clusters_h5)][287:]):
            # if cluster_name=='CWNU_1012':
            #     continue
            delJz_obs,age_array,mu,sigma = prob_func.read_stream_actions(stream_h5_file,cluster_name)
            ...
            results = Parallel(n_jobs=8)(
                delayed(process_age_dual)(age, delJz_obs) for age in tqdm(unique_ages, desc="Computing unique ages")
            )
            ...
            rest = [results[i]['rest'] for i in inv_idx]
            rest_b = [results[i]['rest_b'] for i in inv_idx]
