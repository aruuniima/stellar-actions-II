"""
Compute the evolution of action differences between stellar pairs
that are born within specific spatial separation bins.

Change paths etc at the user configuration block.

This script:
    1. Loads JR/Jphi/Jz, cylindrical positions, and ages.
    2. Converts coordinates to Cartesian.
    3. Identifies stellar pairs born within each separation bin.
    4. For each Δt (1–456 Myr), computes:
         - Absolute change in |ΔJ|
         - Relative change in |ΔJ| / sqrt(J1*J2)
         - Only for pairs that separate >30 pc after 100 Myr.
    5. Saves medians per Δt per bin to an HDF5 file.
    6. Uses a logfile to allow restart.
...
actions_data_path = 'stellar-actions-I/data/actions.h5' #GET DATA FROM  https://www.mso.anu.edu.au/~arunima/stellar-actions-I-data/
